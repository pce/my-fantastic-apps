...
void ofApp::setup() {
    ...
    transformer.setRenderSize(800, 600);
    ...
    // load scenes
    titleScene = (TitleScene*) sceneManager.add(new TitleScene());
    sceneManager.add(new RadialLineScene());
    sceneManager.setup(true); // true = setup all the scenes now (not on the fly)
    ofSetLogLevel("ofxSceneManager", OF_LOG_VERBOSE); // lets see whats going on inside
    
    // start with a specific scene
    ofLog(OF_LOG_NOTICE, "number of scenes:" + ofToString(sceneManager.getNumScenes()));
...
void ofApp::keyPressed(int key) {
    ...
    switch(key) {
        ...
        case '-':
            if(sceneManager.getCurrentScene() == titleScene) {
                // titleScene->removeOne();
            }
            break;
